# 1 "/home/pytree/repos/college-arduino/向心力物理實驗/CentripetalForce/CentripetalForce.ino"
...
void setup() {
    Serial.begin(9600);
    pinMode(counterPin, 0x0);
    attachInterrupt(0, counter, 2); // 計數，以電壓由 HIGH 轉變 LOW 算一次
    // 外部中斷處理函式指令，
    // 參數 1：編號 0 指的是 pin2
...
void loop() {
    ...
    do {
        if (millis() - time >= 1000) { // 每秒計算一次中斷次數
            ...
            n++;
            attachInterrupt(0, counter, 2); // 重啟計數
        }
    } while (n < 11); // 十次就離開迴圈
    Serial.print("\n"); // 換行
    ...
    Serial.print("Force = ");
    Serial.println(Force / 10, 10); // 顯示 10 次的平均
    n = 1;
    SumCycle = 0;
    Force = 0;
}
